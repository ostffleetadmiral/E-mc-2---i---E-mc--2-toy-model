...
Q128.128 addresses this by using integer-only arithmetic with a fixed-point representation. All operations are performed on 256-bit signed integers, with multiplication using exact 512-bit intermediate products and round-to-nearest-even (RNE) truncation. The result is deterministic: the same inputs always produce the same outputs, regardless of platform, compiler, or hardware. The engine is used as the computational backbone for the octonionic physics framework described in a companion paper \cite{ramsey2026framework}, as well as codon routing \cite{ramsey2026codon} and sentience testing \cite{ramsey2026neuraleak}.
...
The fractional resolution of $2^{-128} \approx 2.94 \times 10^{-39}$ provides approximately 38.5 decimal digits of fractional precision, sufficient for exact representation of all fundamental physical constants and their products \cite{higham_2002}.
...
\begin{proof}
The discarded bits $d \in [0, 2^{128} - 1]$. The halfway point is $h = 2^{127}$. Round-to-nearest (with any tie-breaking rule) ensures the rounded result is within $h$ of the exact value, giving an error of at most $h / 2^{128} = 2^{127} / 2^{128} = 2^{-1}$ ULP $= 2^{-129}$ in value. RNE specifically provides an unbiased error distribution (zero mean), but the worst-case bound is the same for any round-to-nearest variant. $\square$
\end{proof}
...
The \texttt{fromRatio} constructor creates a Q128.128 from a rational $n/d$ using a modified round-half-away-from-zero that avoids rounding exact zeros upward:
...
The Q128.128 engine includes RamseyIdentity transforms that encode the framework's structural identity \cite{ramsey2026framework}:
\begin{equation}
...
These transforms are framework-internal algebraic identities, not physical derivations. The first states that the imaginary unit $i$ serves as an algebraic bridge between the energy-momentum relation $E = mc^2$ and its inverse $E = mc^{-2}$. The second expresses $i$ as the real unit $e_0$ projected from the full octonion basis. The transforms are implemented as Q128.128 fixed-point operations that verify the algebraic consistency of the framework's identity chain.
