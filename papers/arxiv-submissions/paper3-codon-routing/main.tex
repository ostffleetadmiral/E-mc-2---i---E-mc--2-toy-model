...
where $S = 100$ is the integer scaling factor (multiplying the molecular weight in g/mol by 100 to obtain an integer coordinate) and $\text{MW}_{\text{base}}$ is the molecular weight of the nucleobase:
\begin{itemize}
...
The routing system uses two related M\"obius transformations:

\textbf{Smith chart} (impedance matching, standard EE):
\begin{equation}
\Gamma_{\text{SC}}(z) = \frac{z - 1}{z + 1}
\end{equation}
with boundaries $\Gamma_{\text{SC}}(0) = -1$, $\Gamma_{\text{SC}}(1) = 0$, $\Gamma_{\text{SC}}(\infty) = +1$.

\textbf{Self-inverse checksum} (E=mc$^2$ $\leftrightarrow$ E=mc$^{-2}$ round-trip):
\begin{equation}
\Gamma(z) = \frac{1 - z}{1 + z}
\end{equation}

\begin{theorem}[Self-Inverse Property]
$\Gamma(\Gamma(z)) = z$, i.e., the M\"obius transformation $\Gamma(z) = (1-z)/(1+z)$ is self-inverse.
\end{theorem}

\begin{proof}
\begin{align*}
\Gamma(\Gamma(z)) &= \frac{1 - \frac{1-z}{1+z}}{1 + \frac{1-z}{1+z}} = \frac{\frac{(1+z) - (1-z)}{1+z}}{\frac{(1+z) + (1-z)}{1+z}} \\
&= \frac{(1+z) - (1-z)}{(1+z) + (1-z)} = \frac{2z}{2} = z \quad \square
\end{align*}
\end{proof}

The self-inverse boundaries are:
\begin{equation}
\Gamma(0) = 1, \quad \Gamma(1) = 0, \quad \Gamma(\infty) = -1
\end{equation}

Note: $\Gamma_{\text{SC}}(z) = -\Gamma(z)$. The Smith chart formula is NOT self-inverse ($\Gamma_{\text{SC}}(\Gamma_{\text{SC}}(z)) = -1/z$), while the self-inverse form is NOT the standard Smith chart. Both are used for their respective purposes.
