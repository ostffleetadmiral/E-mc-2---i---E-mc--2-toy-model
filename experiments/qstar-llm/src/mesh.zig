...
pub const ConnectionPool = struct {
    ...
    pub fn batchSync(self: *ConnectionPool, local_states: []const i128) []i128 {
        var correlations = self.allocator.alloc(i128, self.connections.items.len) catch return &[_]i128{};
...
test "SharedFace set states" {
    ...
    const local = [_]i128{fp.HALF_FP} ** 225;
    const remote = [_]i128{fp.div(fp.fromInt(3), fp.fromInt(10))} ** 225;
...
test "syncSharedFace updates states" {
    var face = SharedFace.init("peer1", .pos_x, 15);
    const local = [_]i128{fp.div(fp.fromInt(1), fp.fromInt(10))} ** 225;
    const remote = [_]i128{fp.div(fp.fromInt(2), fp.fromInt(10))} ** 225;
...
test "SharedFace delta serialization" {
    var face = SharedFace.init("peer1", .pos_x, 15);
    const states = [_]i128{ fp.fromInt(1), fp.fromInt(2), fp.fromInt(3) } ++ [_]i128{0} ** 222;
    face.setLocalStates(&states);

    // Now change a few states
    const states2 = [_]i128{ fp.fromInt(1), fp.fromInt(5), fp.fromInt(3) } ++ [_]i128{0} ** 222;
...
test "SharedFace apply delta" {
    ...
    const values = [_]i128{ fp.fromInt(10), fp.fromInt(20), fp.fromInt(30) };
...
test "SharedFace full serialize/deserialize" {
    var face = SharedFace.init("peer1", .pos_x, 15);
    const states = [_]i128{fp.fromInt(42)} ++ [_]i128{0} ** 224;
    face.setLocalStates(&states);
...
test "ConnectionPool batch sync" {
    ...
    const states = [_]i128{fp.fromInt(1)} ++ [_]i128{0} ** 224;
