//! e8_roots.zig — Explicit construction of the 240 E8 roots.
//!
//! Ports the hardware (E=mc²-i-E=mc⁻²) project's E8 root system into
//! qstar-llm. The E8 root system has exactly 240 roots in 8-dimensional
//! Euclidean space. This module constructs them explicitly using integer
//! coordinates (scaled by 2 to avoid fractions), verifies the count, and
//! connects to the framework's 15 × 16 = 240 identity.
//!
...
//!   Qstar's lattice uses 421 E0 nodes = (15³ - 7) / 8, where 7 = 2³ - 1
//!
//! All arithmetic is integer-only (i8 coordinates, i32 inner products).
//! No floating-point in any path.
//!
//! Ported from: hardware/src/e8_roots.zig
...
/// Find a root by its coordinates. Returns the index if found, null otherwise.
pub fn findRoot(roots: []const Root, target: Root) ?usize {
    for (roots, 0..) |r, i| {
        if (std.mem.eql(i8, &r, &target)) return i;
    }
    return null;
}

/// Compute the Cartan matrix entry A[i][j] = 2(α_i · α_j)/(α_i · α_i).
/// For E8 (simply-laced, norm² = 8): A[i][j] = (α_i · α_j) / 4.
pub fn cartanEntry(a: Root, b: Root) i32 {
    return @divTrunc(innerProduct(a, b), 4);
}
...
test "findRoot locates roots by coordinates" {
    var roots: [ROOT_COUNT]Root = undefined;
    _ = generateAllRoots(&roots);
    // First root should be findable
    const target = roots[0];
    const idx = findRoot(&roots, target);
    try std.testing.expect(idx != null);
    try std.testing.expectEqual(@as(usize, 0), idx.?);
    // Non-existent root should return null
    const nonexistent: Root = .{ 9, 9, 9, 9, 9, 9, 9, 9 };
    try std.testing.expect(findRoot(&roots, nonexistent) == null);
}

test "cartanEntry computes Cartan matrix entries" {
    var roots: [ROOT_COUNT]Root = undefined;
    _ = generateAllRoots(&roots);
    // Diagonal entries should be 2 (A[i][i] = 2(α·α)/(α·α) = 2)
    const diag = cartanEntry(roots[0], roots[0]);
    try std.testing.expectEqual(@as(i32, 2), diag);
}

test "normSquared returns 8 for all E8 roots" {
    var roots: [ROOT_COUNT]Root = undefined;
    _ = generateAllRoots(&roots);
    for (roots) |r| {
        try std.testing.expectEqual(@as(i32, 8), normSquared(r));
    }
}

test "innerProduct is symmetric" {
    var roots: [ROOT_COUNT]Root = undefined;
    _ = generateAllRoots(&roots);
    // Check symmetry for first few pairs
    for (0..20) |i| {
        for (0..20) |j| {
            try std.testing.expectEqual(
                innerProduct(roots[i], roots[j]),
                innerProduct(roots[j], roots[i]),
            );
        }
    }
}

test "e8_roots uses integer types (no f64)" {
    var roots: [ROOT_COUNT]Root = undefined;
    _ = generateAllRoots(&roots);
    const ns = normSquared(roots[0]);
    try std.testing.expect(@TypeOf(ns) == i32);
    const ip = innerProduct(roots[0], roots[1]);
    try std.testing.expect(@TypeOf(ip) == i32);
    const ce = cartanEntry(roots[0], roots[1]);
    try std.testing.expect(@TypeOf(ce) == i32);
}
...
/// Verifies the E8 root count matches the framework's 15 × 16 = 240 identity.
pub fn verifyE8RootCountMatchesFramework() bool {
    return ROOT_COUNT == 240 and 240 == 15 * 16;
}

/// Verifies the D8 root count (112) matches C(8,2) × 4 = 28 × 4.
pub fn verifyD8RootCountMatchesFramework() bool {
    return 112 == 28 * 4;
}

/// Verifies the spinor root count (128) matches 2^7.
pub fn verifySpinorRootCountMatchesFramework() bool {
    return 128 == 1 << 7;
}

test "framework: E8 root count 240 = 15 × 16" {
    try std.testing.expect(verifyE8RootCountMatchesFramework());
}

test "framework: D8 root count 112 = C(8,2) × 4" {
    try std.testing.expect(verifyD8RootCountMatchesFramework());
}

test "framework: spinor root count 128 = 2^7" {
    try std.testing.expect(verifySpinorRootCountMatchesFramework());
}
