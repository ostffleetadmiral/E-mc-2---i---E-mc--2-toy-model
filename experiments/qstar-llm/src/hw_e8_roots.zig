...
/// This connects the E8 root system to the SM fermion count (15) and
/// the SO(10) spinor dimension (16).
pub fn verifyE8RootCountMatchesFramework() bool {
    return ROOT_COUNT == 240 and ROOT_COUNT == 15 * 16;
}

/// Verifies the D8 root count (112) matches C(8,2) × 4 = 28 × 4.
/// This connects the E8 root system to the octonion dimension (8).
pub fn verifyD8RootCount() bool {
    return 112 == 28 * 4; // C(8,2) × 4
}

/// Verifies the spinor root count (128) matches 2^7.
/// This connects the E8 root system to the octonion capacity (2^8 = 256,
/// with 2^7 = 128 spinor roots due to the even parity constraint).
pub fn verifySpinorRootCount() bool {
    return 128 == 1 << 7; // 2^7
}

test "framework: E8 root count matches 15 × 16 = 240" {
    try std.testing.expect(verifyE8RootCountMatchesFramework());
}

test "framework: D8 root count is 112 = C(8,2) × 4" {
    try std.testing.expect(verifyD8RootCount());
}

test "framework: spinor root count is 128 = 2^7" {
    try std.testing.expect(verifySpinorRootCount());
}
