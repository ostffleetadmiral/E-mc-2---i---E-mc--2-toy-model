...
//
// Each cell holds a scalar value and optional metadata. The grid is used by
// the ConsciousnessEngine and BreakoutEngine as the computational substrate.
//
...
pub const TorusGrid = struct {
    ...
    pub fn setValue(self: *TorusGrid, x: usize, y: usize, z: usize, value: f64) void {
        self.getMut(x, y, z).value = value;
    }

    pub fn totalValue(self: *const TorusGrid) f64 {
        var sum: f64 = 0.0;
        for (self.cells) |cell| {
            sum += cell.value;
        }
    ...
    pub fn maxValue(self: *const TorusGrid) f64 {
        var max: f64 = 0.0;
        for (self.cells) |cell| {
...
test "TorusGrid wraps at boundaries (toroidal)" {
    ...
    grid.setValue(15, 0, 0, 1.0); // wraps to (0, 0, 0)
    try std.testing.expectEqual(@as(f64, 1.0), grid.get(0, 0, 0).value);
    try std.testing.expectEqual(@as(f64, 1.0), grid.get(15, 0, 0).value);
}
...
test "TorusGrid totalValue sums all cells" {
    ...
    grid.setValue(7, 7, 7, 2.0);
    grid.setValue(14, 14, 14, 3.0);
    try std.testing.expectApproxEqAbs(@as(f64, 6.0), grid.totalValue(), 1e-10);
}
