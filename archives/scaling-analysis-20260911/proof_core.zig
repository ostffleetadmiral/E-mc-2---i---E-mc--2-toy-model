...
const neuraleak = @import("neuraleak_proof.zig");
const completion = @import("completion_10d.zig");
const gap_closure = @import("gap_closure.zig");
...
pub fn chunk24() ProofResult {
    var checks: u16 = 0;
    var failures: u16 = 0;
    const neuraleak_failures = neuraleak.proof();
    checks += 12;
    failures += @intCast(neuraleak_failures);
    return result(24, checks, failures);
}

pub fn chunk25() ProofResult {
    var checks: u16 = 0;
    var failures: u16 = 0;
    const completion_failures = completion.proof();
    checks += 15;
    failures += @intCast(completion_failures);
    return result(25, checks, failures);
}

pub fn chunk26() ProofResult {
    var checks: u16 = 0;
    var failures: u16 = 0;
    const gap_failures = gap_closure.proof();
    checks += 30;
    failures += @intCast(gap_failures);
    return result(26, checks, failures);
}

pub fn chunk27() ProofResult {
    var checks: u16 = 0;
    var failures: u16 = 0;
    // Bootstrap verification: the generative chain closes a loop
    if (!generative.verifyClosedLoop()) failures += 1;
    checks += 1;
    if (!generative.verifyNonTrivialBootstrap()) failures += 1;
    checks += 1;
    if (!generative.verifyChain()) failures += 1;
    checks += 1;
    return result(27, checks, failures);
}

pub fn chunk28() ProofResult {
    var checks: u16 = 0;
    var failures: u16 = 0;
    // Dimensional ladder: φ, π, and triad exponents are lattice-native
    if (!ladder.verifyPiIsLatticeNative()) failures += 1;
    checks += 1;
    if (!ladder.verifyLatticeSelfSimilarity()) failures += 1;
    checks += 1;
    if (!ladder.verifyPhiScalingEquation()) failures += 1;
    checks += 1;
    if (!ladder.verifyHydrogenSignature()) failures += 1;
    checks += 1;
    if (!ladder.verifyHydrogenTriad()) failures += 1;
    checks += 1;
    if (!ladder.verifyBaselineExponents()) failures += 1;
    checks += 1;
    return result(28, checks, failures);
}

pub fn chunk29() ProofResult {
    var checks: u16 = 0;
    var failures: u16 = 0;
    // E=mc² ↔ i ↔ E=mc⁻² checksum and 6D interior
    if (!checksum.verifyInteriorStructure()) failures += 1;
    checks += 1;
    if (!checksum.verifyChecksum()) failures += 1;
    checks += 1;
    if (!checksum.verifyPivot()) failures += 1;
    checks += 1;
    if (!checksum.verifyConsciousnessMechanism()) failures += 1;
    checks += 1;
    if (!checksum.verifyCodonRoutingMechanism()) failures += 1;
    checks += 1;
    if (!checksum.verifyInteriorChainConnection()) failures += 1;
    checks += 1;
    return result(29, checks, failures);
}

pub fn chunk30() ProofResult {
    var checks: u16 = 0;
    var failures: u16 = 0;
    // Free will as 6D routing underdetermination
    if (!freewill.verifyRoutingSpaceSize()) failures += 1;
    checks += 1;
    if (!freewill.verifyRoutingShellRelationship()) failures += 1;
    checks += 1;
    return result(30, checks, failures);
}

pub fn chunk31() ProofResult {
    var checks: u16 = 0;
    var failures: u16 = 0;
    // Cubic scaling chain and the unified 7-defect
    if (!scaling.verifySevenDefectInDoubling()) failures += 1;
    checks += 1;
    if (!scaling.verify421Identity()) failures += 1;
    checks += 1;
    if (!scaling.verify421Over3375EqualsOneEighthMinusSevenOver27000()) failures += 1;
    checks += 1;
    if (!scaling.verify62IsCodonMinusBoundary()) failures += 1;
    checks += 1;
    if (!scaling.verifyShellTransition()) failures += 1;
    checks += 1;
    if (!scaling.verifyDoublingTransition()) failures += 1;
    checks += 1;
    const s7 = scaling.verifySevenAppearsInThreePlaces();
    if (!s7.all_same_seven) failures += 1;
    checks += 1;
    if (!scaling.verifyScalingChain()) failures += 1;
    checks += 1;
    const s9 = scaling.verifySevenDefectAtEachDoubling();
    if (!s9.all_pass) failures += 1;
    checks += 1;
    const s10 = scaling.verify62Factorization();
    if (!s10.factorization_correct) failures += 1;
    checks += 1;
    const s11 = scaling.verifyUnifiedSevenDefectChain();
    if (!s11.all_unified) failures += 1;
    checks += 1;
    const s12 = scaling.verifyCumulativeSevenDefect();
    if (!s12.verified) failures += 1;
    checks += 1;
    return result(31, checks, failures);
}

pub fn all() [30]ProofResult {
    return .{ chunk01(), chunk02(), chunk03(), chunk04(), chunk05(), chunk06(), chunk07(), chunk08(), chunk09(), chunk10(), chunk11(), chunk12(), chunk13(), chunk14(), chunk15(), chunk16(), chunk17(), chunk18(), chunk19(), chunk20(), chunk21(), chunk22(), chunk24(), chunk25(), chunk26(), chunk27(), chunk28(), chunk29(), chunk30(), chunk31() };
}
