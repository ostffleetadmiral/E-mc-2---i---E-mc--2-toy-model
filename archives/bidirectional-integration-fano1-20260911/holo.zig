...
//! holo.zig - Holographic compression codec (Modules 3 & 4, concretized).
//!
//! Folds an N^3 complex grid (Q128.128 re/im pairs) onto a 16^3 core plus
//! a per-cell e9 winding field organized in scale levels
//! (16^3 -> 32^3 -> ... -> 512^3). Family states (FANO-1-representable:
...
//! (16*2^L)^3 grid without touching higher levels.
//!
//! Adapted from the Q128.128 reference implementation to use the hardware
//! project's `fixed_point.zig` (Q128 with a single i256 `.raw` field) in
//! place of the `q128_128` module. All arithmetic is exact fixed-point;
//! no floating-point types appear in this core module.
...
/// Complex Q128.128 value (64 bytes: two i256 raw words).
pub const Cx = struct {
    re: fixed.Q128,
    im: fixed.Q128,

    pub fn eq(a: Cx, b: Cx) bool {
        return fixed.Q128.eq(a.re, b.re) and fixed.Q128.eq(a.im, b.im);
    }
};
...
/// A hologram: the folded form of an N^3 grid.
pub const Hologram = struct {
    ...
    pub fn hologramBytes(self: *const Hologram) u64 {
        return @sizeOf(Header) + @as(u64, CORE_CELLS) * 64 +
            self.winding_rle.len + self.residuals.len * 64;
    }

    pub fn gridBytes(self: *const Hologram) u64 {
        return @as(u64, self.n) * self.n * self.n * 64;
    }

    /// Node-count ratio (the Module 3 figure): N^3 / 16^3, as a Q128
    /// fixed-point value (no floating-point in core modules).
    pub fn nodeRatio(self: *const Hologram) fixed.Error!fixed.Q128 {
        const nodes: i256 = @as(i256, @as(u64, self.n) * self.n * self.n);
        return try fixed.Q128.fromRatio(nodes, @as(i256, CORE_CELLS));
    }

    /// Byte ratio: raw grid bytes / hologram bytes, as a Q128 fixed-point
    /// value (no floating-point in core modules).
    pub fn byteRatio(self: *const Hologram) fixed.Error!fixed.Q128 {
        const gb: i256 = @as(i256, @intCast(self.gridBytes()));
        const hb: i256 = @as(i256, @intCast(self.hologramBytes()));
        return try fixed.Q128.fromRatio(gb, hb);
    }
};
...
fn rleTotal(stream: []const u8) u64 {
    var it = RleIter{ .stream = stream, .pos = 0 };
    var total: u64 = 0;
    while (it.next()) |pair| total += pair.count;
    return total;
}
...
fn fnvCore(core: []const Cx) u32 {
    ...
    for (core) |v| {
        const bytes = std.mem.asBytes(&v.re.raw) ++ std.mem.asBytes(&v.im.raw);
        for (bytes) |b| {
            h ^= b;
            h *%= 0x01000193;
        }
...
/// Fold an N^3 grid into a hologram. N must be a ladder size or a ladder
...
pub fn fold(alloc: std.mem.Allocator, grid: *const Grid) !Hologram {
    ...
    // ---- dynamics attempt ----
    var ok = true;
    var lists: [MAX_LEVELS + 1]std.ArrayList(u8) = undefined; // [1..levels] used
...
/// Write a Cx as two little-endian i256 raw words (64 bytes total).
fn writeCx(buf: []u8, off: usize, v: Cx) void {
    std.mem.writeInt(u256, buf[off..][0..32], @as(u256, @bitCast(v.re.raw)), .little);
    std.mem.writeInt(u256, buf[off + 32 ..][0..32], @as(u256, @bitCast(v.im.raw)), .little);
}

/// Read a Cx from two little-endian i256 raw words (64 bytes total).
fn readCx(buf: []const u8, off: usize) !Cx {
    if (off + 64 > buf.len) return error.CorruptHologram;
    return .{
        .re = fixed.Q128.fromRaw(@as(i256, @bitCast(std.mem.readInt(u256, buf[off..][0..32], .little)))),
        .im = fixed.Q128.fromRaw(@as(i256, @bitCast(std.mem.readInt(u256, buf[off + 32 ..][0..32], .little)))),
    };
}
...
/// N^3 grid (perimeter shell included for odd N).
...
pub fn deserialize(alloc: std.mem.Allocator, bytes: []const u8) !Hologram {
    ...
    const need = @sizeOf(Header) + @as(u64, CORE_CELLS) * 64 + h.winding_len + h.residual_count * 64;
...
pub fn generateFamily(alloc: std.mem.Allocator, n: u32, rng: std.Random, winding_rate: u8) !Grid {
    ...
    var prev = try Grid.init(alloc, CORE_SIDE);
    for (prev.data) |*v| {
        v.re = fixed.Q128.fromInteger(@as(i256, rng.int(u32)));
        v.im = fixed.Q128.fromInteger(@as(i256, rng.int(u32)));
    }
    var s: u5 = 1;
    while (s <= levels) : (s += 1) {
...
// ---------------------------------------------------------------------
// Tests — exact integer assertions only (no floating-point).
// ---------------------------------------------------------------------
...
test "partial unfold at every level matches the family sub-lattice" {
    var arena = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena.deinit();
    const alloc = arena.allocator();
    ...
    // level 1: the full 32^3 ladder — native coordinates in g (0..31)
    const r1 = try unfoldAt(alloc, &h, 1, false);
    try testing.expectEqual(@as(u32, 32), r1.n);
    z = 0;
    while (z < 32) : (z += 1) {
        var y: u32 = 0;
        while (y < 32) : (y += 1) {
            var x: u32 = 0;
            while (x < 32) : (x += 1) {
                try testing.expect(r1.get(x, y, z).eq(g.get(x, y, z)));
            }
        }
    }
    var rfull = try unfold(alloc, &h);
    try expectGridEq(&g, &rfull);
}
...
test "pure family state achieves holographic ratio (Module 3)" {
    var arena = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena.deinit();
    const alloc = arena.allocator();
    ...
    var h = try fold(alloc, &g);
    try testing.expectEqual(Mode.dynamics, h.mode);
    // node ratio is exactly the Module 3 figure: 65^3 / 16^3 = 274625 / 4096
    // Integer part is 67 (274625 / 4096 = 67.0498...).
    const nr = try h.nodeRatio();
    try testing.expectEqual(@as(i256, 67), nr.toInteger());
    // byte ratio > 50 (grid bytes dwarf the ~256 KB hologram)
    const br = try h.byteRatio();
    try testing.expect(br.toInteger() > 50);
    var r = try unfold(alloc, &h);
    try expectGridEq(&g, &r);
}
...
test "ladder and level helpers" {
    ...
    try testing.expectEqual(@as(u64, 787969), @as(u64, 513) * 513 * 513 - 512 * 512 * 512);
}

test "rot90 has period 4 and is exact" {
    const v = Cx{ .re = fixed.Q128.fromI64(3), .im = fixed.Q128.fromI64(7) };
    try testing.expect(rotPow(v, 4).eq(v));
    try testing.expect(rotPow(v, 0).eq(v));
    const r1 = rot90(v);
    try testing.expect(fixed.Q128.eq(r1.re, v.im.neg()));
    try testing.expect(fixed.Q128.eq(r1.im, v.re));
}

test "16^3 core is extracted correctly from a family grid" {
    var arena = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena.deinit();
    const alloc = arena.allocator();
    var prng = std.Random.DefaultPrng.init(21);
    const g = try generateFamily(alloc, 33, prng.random(), 0);
    const h = try fold(alloc, &g);
    try testing.expectEqual(Mode.dynamics, h.mode);
    // For N=33 the ladder is 32, levels=1, stride=2. The core samples
    // every even-coordinate cell of the 32^3 ladder.
    const stride: u32 = 2;
    var z: u32 = 0;
    while (z < CORE_SIDE) : (z += 1) {
        var y: u32 = 0;
        while (y < CORE_SIDE) : (y += 1) {
            var x: u32 = 0;
            while (x < CORE_SIDE) : (x += 1) {
                const core_v = h.core[x + y * CORE_SIDE + z * CORE_SIDE * CORE_SIDE];
                try testing.expect(core_v.eq(g.get(x * stride, y * stride, z * stride)));
            }
        }
    }
}

test "winding vectors are computed correctly for a known family" {
    var arena = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena.deinit();
    const alloc = arena.allocator();
    // Build a 32^3 family by hand: core all zero, every odd cell = rot90
    // of its even-floor neighbor (winding 1 everywhere).
    var g = try Grid.init(alloc, 32);
    var z: u32 = 0;
    while (z < 32) : (z += 1) {
        var y: u32 = 0;
        while (y < 32) : (y += 1) {
            ...
            while (x < 32) : (x += 1) {
                if ((x & 1) == 0 and (y & 1) == 0 and (z & 1) == 0) {
                    g.set(x, y, z, .{ .re = fixed.Q128.zero, .im = fixed.Q128.zero });
                } else {
                    const nb = g.get(x & ~@as(u32, 1), y & ~@as(u32, 1), z & ~@as(u32, 1));
                    g.set(x, y, z, rotPow(nb, 1));
                }
            }
        }
    }
    var h = try fold(alloc, &g);
    try testing.expectEqual(Mode.dynamics, h.mode);
    // All windings should be 1 (winding 0 only if neighbor is zero, but
    // rotPow(zero, 1) == zero so windingFor returns 0 for zero neighbors).
    // Decode the level-1 RLE stream and verify every value is 0 or 1 and
    // the total count equals oddCellCount(32) = 28672.
    try testing.expectEqual(@as(u64, 28672), rleTotal(h.winding_rle[h.level_woff[1]..h.level_woff[2]]));
    var it = RleIter{ .stream = h.winding_rle, .pos = h.level_woff[1] };
    var seen: u64 = 0;
    while (seen < oddCellCount(32)) {
        const pair = it.next() orelse return error.CorruptHologram;
        try testing.expect(pair.value <= 1);
        seen += pair.count;
    }
    // Round-trip must reproduce the grid exactly.
    var r = try unfold(alloc, &h);
    try expectGridEq(&g, &r);
}

test "serialization preserves core hash and defect for odd perimeter" {
    var arena = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena.deinit();
    const alloc = arena.allocator();
    var prng = std.Random.DefaultPrng.init(77);
    var g = try generateFamily(alloc, 17, prng.random(), 15);
    var h = try fold(alloc, &g);
    try testing.expectEqual(Mode.dynamics, h.mode);
    try testing.expect(h.perimeter_count > 0);
    const bytes = try serialize(alloc, &h);
    var h2 = try deserialize(alloc, bytes);
    try testing.expectEqual(h.perimeter_count, h2.perimeter_count);
    try testing.expectEqual(h.defect, h2.defect);
    // 17^3 = 4913 perimeter + ladder cells; perimeter_count = 17^3 - 16^3
    try testing.expectEqual(@as(u64, 4913 - 4096), h.perimeter_count);
    var r = try unfold(alloc, &h2);
    try expectGridEq(&g, &r);
}
